...
The tag goes on **every** declaration whose union states the result's content, not merely the
first; scaffolding — helper definitions, hypothesis bundles, proof-local lemmas — stays untagged.
-/
...
match `@[stacks …]`, but Mathlib's `defsWithUnderscore` linter inspects the declaration name, so
the internal name must be lowerCamelCase. -/
