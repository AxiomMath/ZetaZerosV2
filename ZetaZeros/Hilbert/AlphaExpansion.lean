...
The analytic ingredients `lem_alpha_expansion` needs on top of the algebraic identity `bigF_eq`.
...
This is the reusable core of `lem_alpha_expansion`, applied once per family (`fz`, `gz`, `hz`). -/
...
/-- **The set-integral form of `lem_symmetric_inner_real`.** The pairing of two symmetric functions
over the symmetric interval is real. -/
...
/-- `lem_alpha_expansion`, restated for a single vector. Definitionally the same statement. -/
...
/-- **The normalising constant is positive** (`lem_normaliser_pos`).
...
/-- **The tensor squares pair as the square of the inner product** (`lem_Psi_orthonormal`),
hence are orthonormal in `L²(I²)`.

Note this needs no Fubini: the double integral factors by pulling a constant out of each integral,
the same route `integral_double_finsetSum` takes. -/
...
/-- `lem_g_h_norm_diff` at the level of `L²` elements. -/
...
The declarations below cite the Mathlib lemmas they lean on; the new ingredient over standard
Gram-Schmidt is that every vector stays in the prescribed REAL subspace, which is what carries the
paper's symmetry.
...
/-- **A symmetric adapted basis exists** (`lem_adapted_basis_exists`).
...
/-- The rescaling is injective: it is affine with non-zero linear coefficient. Together with
`rescaledMult_rescale` this is the multiplicity-preserving bijection onto `rescaledZerosFinset`,
which is by definition the image. -/
...
`ZetaZeros.sum_sq_norm_inner_eq_norm_sq_of_mem_span` is the `Fin N` case; nothing in its proof used
the index being `Fin N`. -/
...
DELIBERATELY UNTAGGED. This is the abstract statement: it mentions no admissible `eta`, no
`bigF` and no `alphaCoeff`, so it does not state `lem_bessel_F` — the same reason the general
Gram-Schmidt lemma is untagged. What the instantiation still needs is that `bigF` and the tensor
squares are `MemLp 2` for the PRODUCT measure; the orthonormality hypothesis is already discharged
by `integral_tensor_square_pairing`.
-/
...
/-- **The normalised cutoff test function is `1/2`-admissible** (`lem_eta_psi_admissible`).

Three of the four fields are `lem_eta_psi_smooth` and `lem_f_psi_integral` read off directly. Only
square-integrability is new, and it is immediate once the function is known to be continuous with
compact support -- which is why `HasCompactSupport` was worth stating there rather than leaving the
support condition purely pointwise. -/
...
`lem_kernel_sum_identity`. The double sum over the rescaled zeros and the double sum over the actual
...
Stated without an admissibility hypothesis on `eta`: nothing in a reindexing can use it, and
dropping it is a generalisation. -/
...
/-- **A symmetric adapted orthonormal basis exists** (`lem_adapted_basis_exists`).

Stated without a conjugation-invariance hypothesis on `(Z, m)`: the construction lists `R₂`, `S`,
`R₁`, `S` and Gram-Schmidts, and none of that uses it. Dropping it generalises. -/
...
`lem_Q_psi_hat`. The convolution theorem, at a COMPLEX frequency. The prover's move worth keeping is
to abstract the character: the statement holds for ANY continuous `E : ℝ → ℂ` with
...
`lem_second_moment`. A double integral whose integrand is a finite double sum with factored terms
equals the double sum of the products of the single integrals. Also delegated, also abstract. -/
...
`lem_bessel_F`. The abstract inequality is `sum_sq_pairing_le_integral_norm_sq`, and the
orthonormality of the tensor squares is `lem_Psi_orthonormal`. What is left is the integrability,
and `lem_bessel_F` carries NO integrability hypothesis -- so it has to be proved, not assumed.
Adding one would be a weakening.
...
Realness of the coefficients is NOT needed, which is the one place this improves on the usual
proof. That proof invokes `lem_alpha_real` to turn `|alpha_j|^2` into `alpha_j^2`; but the norm
form is the STRONGER inequality and `(re a)^2 <= ‖a‖^2` holds for every complex `a`, so the
real-part form follows with no appeal to realness, no conjugation-invariance and no symmetry
hypothesis. That matters practically: `lem_alpha_real` wants POINTWISE `IsSymmetric`, whereas a
Gram-Schmidt basis is only symmetric almost everywhere. -/
...
/-- **Bessel's inequality for the kernel, in norm form.** Stronger than the real-part form of
`lem_bessel_F`, and what actually gets proved. -/
...
`lem_Z_T_conj`. The composite symmetry `rho ↦ 1 - conj rho` is what makes the rescaled zeros
conjugation-invariant, and both halves are proved: `lem_order_conj` for conjugation and
`lem_order_one_sub` for the reflection.
